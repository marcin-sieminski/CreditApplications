...
public class PageRepository : IRepository<Page>
{
    ...
    private readonly DbSet<Page> _entities;

    public PageRepository(CreditApplicationsDbContext context)
    {
        _context = context;
        _entities = context.Set<Page>();
    }
    ...
    public async Task<int> Update(Page entity)
    {
        if (entity == null)
        {
            throw new ArgumentNullException("entity");
        }
        var dbEntity = _context.Pages.AsNoTracking().FirstOrDefault(x => x.Id == entity.Id);
